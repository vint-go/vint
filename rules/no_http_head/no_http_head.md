...
- Category: `lint/correctness/noHttpHead`
...
Disallow calling `net/http.Head` without a context. The function `http.Head` does not accept a `context.Context` parameter, which prevents the caller from controlling cancellation, deadlines, and tracing. Use `net/http.NewRequestWithContext` to create a request with context, then execute it with `(*net/http.Client).Do`.
...
func main() {
    // http.Head does not accept a context
