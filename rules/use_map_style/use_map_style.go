...
func (r *EnforceMapStyleRule) Configure(arguments lint.Arguments) error {
	...
	if !ok {
		return fmt.Errorf("invalid argument '%v' for 'useMapStyle' rule. Expecting string, got %T", arguments[0], arguments[0])
	}

	var err error
	r.enforceMapStyle, err = mapStyleFromString(enforceMapStyle)
	if err != nil {
		return fmt.Errorf("invalid argument to the useMapStyle rule: %w", err)
	}
...
func (*EnforceMapStyleRule) Group() string {
	return "style"
}
...
func (*EnforceMapStyleRule) CacheTier() rulecache.CacheTier {
