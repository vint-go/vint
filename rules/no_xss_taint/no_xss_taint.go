...
// NoXssTaintRule detects Cross-Site Scripting (XSS) vulnerabilities via taint
// analysis. It traces the flow of untrusted data from HTTP request sources
// (such as query parameters, form values) to HTTP response sinks (such as
// w.Write, fmt.Fprintf). It performs interprocedural analysis to detect cases
// where user-controlled input is written to HTTP responses without proper
// encoding or sanitization.
type NoXssTaintRule struct{}

// Apply applies the rule to the given file.
func (r *NoXssTaintRule) Apply(file *lint.File, _ lint.Arguments) []lint.Failure {
	...
	for _, decl := range file.AST.Decls {
		...
		if isHTTPHandler(fn) {
			...
			// Find the ResponseWriter parameter name for sink detection.
			rwParam := findHTTPResponseWriterParam(fn)

			// Phase 3: Check for direct sinks within this function.
			checkXSSSinks(fn.Body, tainted, rwParam, funcDecls, onFailure)

			// Phase 4: Interprocedural - check calls to local functions with tainted args.
			checkInterproceduralXSSSinks(fn.Body, tainted, rwParam, funcDecls, onFailure)
		}
	}
...
func (*NoXssTaintRule) Name() string {
	return "noXssTaint"
}

// Group returns the rule group.
func (*NoXssTaintRule) Group() string {
	return "security"
}

// CacheTier returns the cache tier for this rule.
func (*NoXssTaintRule) CacheTier() rulecache.CacheTier {
...
func isTaintedExpr(expr ast.Expr, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl) bool {
	...
	case *ast.CallExpr:
		...
		// Type conversions like []byte(expr), string(expr) propagate taint.
		if isTypeConversion(e) {
			if len(e.Args) > 0 {
				return isTaintedExpr(e.Args[0], tainted, funcDecls)
			}
		}
	case *ast.BinaryExpr:
...
// findHTTPResponseWriterParam returns the name of the http.ResponseWriter parameter.
func findHTTPResponseWriterParam(fn *ast.FuncDecl) string {
	for _, field := range fn.Type.Params.List {
		typeStr := exprToString(field.Type)
		if typeStr == "http.ResponseWriter" && len(field.Names) > 0 {
			return field.Names[0].Name
		}
	}
	return ""
...
// isTypeConversion checks if a call expression is a type conversion (e.g., []byte(x), string(x)).
func isTypeConversion(call *ast.CallExpr) bool {
	...
	case *ast.ArrayType:
		...
		return true
	case *ast.Ident:
		// string(x), int(x), etc. - but only built-in type conversions
...
// isResponseWriteSink checks if a call expression writes tainted data to an HTTP response.
// It returns true and the index of the tainted argument if the call is a response write sink.
func isResponseWriteSink(call *ast.CallExpr, rwName string, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl) bool {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if ok {
		// w.Write(taintedData) - method call on ResponseWriter
		if ident, ok := sel.X.(*ast.Ident); ok && ident.Name == rwName {
			if sel.Sel.Name == "Write" {
				// Check if the argument is tainted
				if len(call.Args) > 0 && isTaintedExpr(call.Args[0], tainted, funcDecls) {
					return true
				}
			}
		}

		// fmt.Fprintf(w, ..., tainted, ...) or fmt.Fprint(w, tainted)
		if pkg, ok := sel.X.(*ast.Ident); ok && pkg.Name == "fmt" {
			switch sel.Sel.Name {
			case "Fprintf", "Fprint", "Fprintln":
				// First arg is the writer, check if it's the ResponseWriter
				if len(call.Args) >= 2 {
					if writerIdent, ok := call.Args[0].(*ast.Ident); ok && writerIdent.Name == rwName {
						// Check if any subsequent argument is tainted
...
// checkXSSSinks walks a function body looking for tainted data flowing into HTTP response sinks.
func checkXSSSinks(body *ast.BlockStmt, tainted map[string]bool, rwName string, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	if body == nil || rwName == "" {
		return
	}
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		if isResponseWriteSink(call, rwName, tainted, funcDecls) {
			onFailure(call, "potential XSS: tainted data from user input flows to HTTP response")
		}
...
// checkInterproceduralXSSSinks checks calls to local functions that receive
// tainted args and the ResponseWriter, and contain XSS sinks internally.
func checkInterproceduralXSSSinks(body *ast.BlockStmt, tainted map[string]bool, rwName string, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	if body == nil || rwName == "" {
		return
	}
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		// Also find which parameter is the ResponseWriter in the callee.
		calleeRW := ""
		paramIdx := 0
		for _, field := range fn.Type.Params.List {
			typeStr := exprToString(field.Type)
			for _, name := range field.Names {
				if paramIdx < len(call.Args) {
					// Check if the argument passed is the ResponseWriter variable
					if argIdent, ok := call.Args[paramIdx].(*ast.Ident); ok && argIdent.Name == rwName {
						if typeStr == "http.ResponseWriter" {
							calleeRW = name.Name
						}
					}
		...
		// Check if callee contains XSS sinks with tainted data.
		if calleeRW != "" {
			checkXSSSinks(fn.Body, calleeTainted, calleeRW, funcDecls, func(_ ast.Node, msg string) {
