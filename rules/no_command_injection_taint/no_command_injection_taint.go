package no_command_injection_taint
...
// NoCommandInjectionTaintRule detects command injection vulnerabilities
// by tracing tainted data from HTTP request sources to command execution sinks.
// It performs interprocedural analysis to detect cases where user-controlled
// data flows through multiple functions before reaching a command execution call.
type NoCommandInjectionTaintRule struct{}

// Apply applies the rule to the given file.
func (r *NoCommandInjectionTaintRule) Apply(file *lint.File, _ lint.Arguments) []lint.Failure {
...
func (*NoCommandInjectionTaintRule) Name() string {
	return "noCommandInjectionTaint"
}

// Group returns the rule group.
func (*NoCommandInjectionTaintRule) Group() string {
	return "security"
}

// CacheTier returns the cache tier for this rule.
func (*NoCommandInjectionTaintRule) CacheTier() rulecache.CacheTier {
...
// isCommandSink checks if a call expression is a command execution sink.
func isCommandSink(call *ast.CallExpr) (bool, string) {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return false, ""
	}
	...
	case "exec":
		switch sel.Sel.Name {
		case "Command":
			return true, "exec.Command"
		case "CommandContext":
			return true, "exec.CommandContext"
		}
	case "syscall":
		switch sel.Sel.Name {
		case "Exec":
			return true, "syscall.Exec"
		case "ForkExec":
			return true, "syscall.ForkExec"
		case "StartProcess":
			return true, "syscall.StartProcess"
		}
	case "os":
		if sel.Sel.Name == "StartProcess" {
			return true, "os.StartProcess"
		}
	}
...
// checkSinks walks a function body looking for tainted data flowing into command sinks.
func checkSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		isSink, sinkName := isCommandSink(call)
		if !isSink {
			return true
		}
		...
		for _, arg := range call.Args {
			if isTaintedExpr(arg, tainted, funcDecls) {
				onFailure(call, "potential command injection: tainted data from user input flows to "+sinkName)
				return false
			}
		}
...
// and contain command sinks internally.
func checkInterproceduralSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		// Check if callee contains command sinks with tainted data.
