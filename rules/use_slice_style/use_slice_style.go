...
func (r *EnforceSliceStyleRule) Configure(arguments lint.Arguments) error {
	...
	if !ok {
		return fmt.Errorf("invalid argument '%v' for 'useSliceStyle' rule. Expecting string, got %T", arguments[0], arguments[0])
	}

	var err error
	r.enforceSliceStyle, err = sliceStyleFromString(enforceSliceStyle)
	if err != nil {
		return fmt.Errorf("invalid argument to the useSliceStyle rule: %w", err)
	}
	return nil
}
...
func (*EnforceSliceStyleRule) Name() string {
	return "useSliceStyle"
}

// Group returns the rule group.
func (*EnforceSliceStyleRule) Group() string {
	return "style"
}
