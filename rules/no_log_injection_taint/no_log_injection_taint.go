package no_log_injection_taint
...
// NoLogInjectionTaintRule detects log injection vulnerabilities
// by tracing tainted data from HTTP request sources to logging sinks.
// It performs interprocedural analysis to detect cases where user-controlled
// data flows through multiple functions before reaching a logging call.
type NoLogInjectionTaintRule struct{}

// Apply applies the rule to the given file.
func (r *NoLogInjectionTaintRule) Apply(file *lint.File, _ lint.Arguments) []lint.Failure {
...
func (*NoLogInjectionTaintRule) Name() string {
	return "noLogInjectionTaint"
}

// Group returns the rule group.
func (*NoLogInjectionTaintRule) Group() string {
	return "security"
}

// CacheTier returns the cache tier for this rule.
func (*NoLogInjectionTaintRule) CacheTier() rulecache.CacheTier {
...
// isLogSink checks if a call expression is a logging sink.
// Returns true and the sink name if the expression is a logging call.
func isLogSink(call *ast.CallExpr) (bool, string) {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return false, ""
	}
	...
	case "log":
		...
		case "Print", "Printf", "Println",
			...
			return true, "log." + sel.Sel.Name
		}
	case "slog":
		switch sel.Sel.Name {
		case "Info", "Warn", "Error", "Debug",
			"InfoContext", "WarnContext", "ErrorContext", "DebugContext":
			return true, "slog." + sel.Sel.Name
		}
	}
...
// isSlogTypedAttr checks if a call expression is a slog typed attribute
// constructor like slog.String, slog.Int, etc. These sanitize the value
// by encoding it properly in structured logging.
func isSlogTypedAttr(call *ast.CallExpr) bool {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return false
	}
...
// checkSinks walks a function body looking for tainted data flowing into logging sinks.
func checkSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		isSink, sinkName := isLogSink(call)
		if !isSink {
			return true
		}

		for _, arg := range call.Args {
			// Skip slog typed attribute calls like slog.String("key", val) -
			// these properly encode values in structured logging.
			if argCall, ok := arg.(*ast.CallExpr); ok && isSlogTypedAttr(argCall) {
				continue
			}
			if isTaintedExpr(arg, tainted, funcDecls) {
				onFailure(call, "potential log injection: tainted data from user input flows to "+sinkName)
				return false
			}
		}
...
// and contain logging sinks internally.
func checkInterproceduralSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		// Check if callee contains logging sinks with tainted data.
