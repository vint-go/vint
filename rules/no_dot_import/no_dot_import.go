...
func (r *NoDotImportRule) Configure(arguments lint.Arguments) error {
	...
	if !ok {
		return fmt.Errorf("invalid argument to the noDotImport rule. Expecting a k,v map, got %T", arguments[0])
	}
	...
	for k, v := range args {
		...
		pkgs, ok := v.([]any)
		if !ok {
			return fmt.Errorf("invalid argument to the noDotImport rule, []string expected. Got '%v' (%T)", v, v)
		}
		for _, p := range pkgs {
			pkg, ok := p.(string)
			if !ok {
				return fmt.Errorf("invalid argument to the noDotImport rule, string expected. Got '%v' (%T)", p, p)
			}
