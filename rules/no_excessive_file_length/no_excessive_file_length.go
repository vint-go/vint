package no_excessive_file_length
...
// NoExcessiveFileLengthRule lints the number of lines in a file.
type NoExcessiveFileLengthRule struct {
...
func (r *NoExcessiveFileLengthRule) Apply(file *lint.File, _ lint.Arguments) []lint.Failure {
	if r.max <= 0 {
...
func (r *NoExcessiveFileLengthRule) Configure(arguments lint.Arguments) error {
	if len(arguments) < 1 {
	...
	if !ok {
		return fmt.Errorf(`invalid argument to the "noExcessiveFileLength" rule. Expecting a k,v map, got %T`, arguments[0])
	}
	for k, v := range argKV {
		switch {
		...
		case isRuleOption(k, "skipComments"):
			...
			if !ok {
				return fmt.Errorf(`invalid configuration value for skip comments in "noExcessiveFileLength" rule; need bool but got %T`, v)
			}
			r.skipComments = skipComments
		case isRuleOption(k, "skipBlankLines"):
			skipBlankLines, ok := v.(bool)
			if !ok {
				return fmt.Errorf(`invalid configuration value for skip blank lines in "noExcessiveFileLength" rule; need bool but got %T`, v)
			}
...
func (*NoExcessiveFileLengthRule) Name() string {
	return "noExcessiveFileLength"
}

// Group returns the rule group.
func (*NoExcessiveFileLengthRule) Group() string {
	return "complexity"
}
...
func (*NoExcessiveFileLengthRule) CacheTier() rulecache.CacheTier {
