package no_path_traversal_taint
...
// NoPathTraversalTaintRule detects path traversal vulnerabilities
// by tracing tainted data from HTTP request sources to filesystem access sinks.
// It performs interprocedural analysis to detect cases where user-controlled
// file paths flow through multiple functions before reaching file operations
// such as os.Open, os.ReadFile, os.Create, or os.WriteFile.
type NoPathTraversalTaintRule struct{}

// Apply applies the rule to the given file.
func (r *NoPathTraversalTaintRule) Apply(file *lint.File, _ lint.Arguments) []lint.Failure {
...
func (*NoPathTraversalTaintRule) Name() string {
	return "noPathTraversalTaint"
}

// Group returns the rule group.
func (*NoPathTraversalTaintRule) Group() string {
	return "security"
}

// CacheTier returns the cache tier for this rule.
func (*NoPathTraversalTaintRule) CacheTier() rulecache.CacheTier {
...
func isTaintPropagatingCall(call *ast.CallExpr, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl) bool {
	...
	propagators := map[string]map[string]bool{
		...
		"filepath": {"Join": true, "Clean": true, "Base": true},
		"path":     {"Join": true},
	}
...
// isPathTraversalSink checks if a call expression is a filesystem access sink.
// Returns true and the sink name if the expression is a file operation call.
func isPathTraversalSink(call *ast.CallExpr) (bool, string) {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return false, ""
	}
	...
	case "os":
		...
		case "Open":
			return true, "os.Open"
		case "OpenFile":
			return true, "os.OpenFile"
		case "ReadFile":
			return true, "os.ReadFile"
		case "WriteFile":
			return true, "os.WriteFile"
		case "Create":
			return true, "os.Create"
		case "Mkdir":
			return true, "os.Mkdir"
		case "MkdirAll":
			return true, "os.MkdirAll"
		case "Remove":
			return true, "os.Remove"
		case "RemoveAll":
			return true, "os.RemoveAll"
		case "Rename":
			return true, "os.Rename"
		case "Stat":
			return true, "os.Stat"
		case "Lstat":
			return true, "os.Lstat"
		case "Chmod":
			return true, "os.Chmod"
		case "Chown":
			return true, "os.Chown"
		case "Link":
			return true, "os.Link"
		case "Symlink":
			return true, "os.Symlink"
		case "Truncate":
			return true, "os.Truncate"
		}
	case "ioutil":
		switch sel.Sel.Name {
		case "ReadFile":
			return true, "ioutil.ReadFile"
		case "WriteFile":
			return true, "ioutil.WriteFile"
		case "ReadDir":
			return true, "ioutil.ReadDir"
		}
	}
...
// checkSinks walks a function body looking for tainted data flowing into filesystem sinks.
func checkSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		isSink, sinkName := isPathTraversalSink(call)
		if !isSink {
			return true
		}
		...
		for _, arg := range call.Args {
			if isTaintedExpr(arg, tainted, funcDecls) {
				onFailure(call, "potential path traversal: tainted data from user input flows to "+sinkName)
				return false
			}
		}
...
// and contain filesystem sinks internally.
func checkInterproceduralSinks(body *ast.BlockStmt, tainted map[string]bool, funcDecls map[string]*ast.FuncDecl, onFailure func(ast.Node, string)) {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		// Check if callee contains filesystem sinks with tainted data.
