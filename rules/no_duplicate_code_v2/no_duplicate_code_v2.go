...
// Note: THIS IS WIP implementation, not yet ready
// for anything!!!
...
var (
	_ lint.Rule             = (*NoDuplicateCodeV2Rule)(nil)
	_ lint.AggregatingRule  = (*NoDuplicateCodeV2Rule)(nil)
	_ lint.ConfigurableRule = (*NoDuplicateCodeV2Rule)(nil)
	_ lint.Grouped          = (*NoDuplicateCodeV2Rule)(nil)
)

// NoDuplicateCodeV2Rule detects duplicate code across files using
// a suffix tree algorithm with GC-friendly flat data structures.
type NoDuplicateCodeV2Rule struct {
	threshold int
	mu        sync.Mutex
...
func (*NoDuplicateCodeV2Rule) Name() string { return "noDuplicateCodeV2" }

// Group returns the rule group.
func (*NoDuplicateCodeV2Rule) Group() string { return "complexity" }

// Configure validates and applies rule configuration.
func (r *NoDuplicateCodeV2Rule) Configure(arguments lint.Arguments) error {
	if len(arguments) < 1 {
	...
	if !ok {
		...
		if !ok {
			return fmt.Errorf(`invalid argument to the "noDuplicateCodeV2" rule, expecting a k,v map or int64, got %T`, arguments[0])
		}
	...
	for k, v := range argKV {
		if normalizeOption(k) == normalizeOption("threshold") {
			...
			if !ok || threshold < 0 {
				return fmt.Errorf(`invalid configuration value for threshold in "noDuplicateCodeV2" rule; need positive int64 but got %T`, v)
			}
			r.threshold = int(threshold)
		}
	}
...
func (r *NoDuplicateCodeV2Rule) Apply(_ *lint.File, _ lint.Arguments) []lint.Failure {
	return nil
}
...
func (r *NoDuplicateCodeV2Rule) Collect(file *lint.File, _ lint.Arguments) {
...
func (r *NoDuplicateCodeV2Rule) Finalize() []lint.Failure {
	if r.threshold == 0 {
		r.threshold = defaultThreshold
	}
...
func (r *NoDuplicateCodeV2Rule) fileForIndex(idx int) string {
